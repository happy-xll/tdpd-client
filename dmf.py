...
def calc_dmf_factor(year, month, patch=False, middata_group=None, target_group=None):
    """
    计算同比系数（与去年同期比较）
    ...
    for _, row in params.iterrows():
        ...
        df_mon_ = load_param_value_remote(pam_oo.get('pk_param'), YEAR_last, month, False, 'vm')
...
def batch_calc_range(start_ym, end_ym, middata_group=None, target_group=None, patch=False):
    ...
    from lib_tdpd import get_dmf_yoy_config

    # 如果未指定，从配置文件读取
    config = get_dmf_yoy_config()
    if middata_group is None:
    ...
    while current <= end:
        ...
        print(f"\n>>> 正在计算 {year}-{month:02d} ...")
        try:
            calc_dmf_factor(year, month, patch, middata_group, target_group)
...
if __name__ == '__main__':
    ...
    # 从配置文件读取默认值
    from lib_tdpd import get_dmf_yoy_config, get_middata_group
    default_config = get_dmf_yoy_config()

    # 命令行执行方式（不用此方式执行可把225-239行注释掉）
    parser = argparse.ArgumentParser(description='计算同比系数')
    ...
    calc_dmf_factor(args.year, args.month, args.patch, args.middata_group, args.target_group)
